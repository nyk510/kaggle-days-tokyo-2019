...
class ClassificationComposer:
    def __init__(self, molecule: Molecule, suffix=None, simple=False):
        ...
        self.root_dir = root_dir
        self.raw_feature = MoleculeFeature(molecule, root_dir=root_dir)
        for h in self.raw_feature.logger.handlers:
            h.setLevel('DEBUG')
        self.entry_feature = MoleculeFeature(user_merge_molecule, parent=self.raw_feature)
        self.entry_feature.logger.setLevel('DEBUG')
    ...
    def fit(self, train_df, y, test_df):
        """
        学習の実行
